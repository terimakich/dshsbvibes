...
class YouTubeAPI:
    ...
    async def download(self, link: str, mystic, video: Union[bool, str] = None,
        ...
        if songvideo and format_id and title:
            ...
            file_ext = "mp4"
            print(f"Trying Maybechiku API for songvideo: {full_link}")
            ...
            if not downloaded_file: # Fallback to yt-dlp if new API fails or file not found
                ...
                def song_video_dl_sync():
                    ...
                    try:
                        ...
                        print(f"yt-dlp song_video_dl_sync failed for {full_link}: {e}")
                        return None
                ...
                if not downloaded_file:
                    print(f"yt-dlp fallback failed for {full_link} (songvideo).")

        elif songaudio and format_id and title:
            # Case: Specific audio format download (e.g., from a /formats command)
            file_ext = "mp3"
            print(f"Trying Maybechiku API for songaudio: {full_link}")
            downloaded_file, direct = await _download_from_maybechiku_api(full_link, file_ext, videoid)
            ...
            if not downloaded_file: # Fallback to yt-dlp if new API fails or file not found
                ...
                def song_audio_dl_sync():
                    ...
                    try:
                        ...
                        print(f"yt-dlp song_audio_dl_sync failed for {full_link}: {e}")
                        return None
                ...
                direct = True # yt-dlp local download is always direct
                if not downloaded_file:
                    print(f"yt-dlp fallback failed for {full_link} (songaudio).")

        elif video:
            # Case: General video download (best quality up to 720p/1280p)
            file_ext = "mp4"
            print(f"Trying Maybechiku API for video: {full_link}")
            downloaded_file, direct = await _download_from_maybechiku_api(full_link, file_ext, videoid)
                ...
                if await is_on_off(1): # Check if direct download (local server download) is enabled
                    ...
                    def video_dl_sync():
                        ...
                        try:
                            ...
                            print(f"yt-dlp video_dl_sync failed for {full_link}: {e}")
                            return None
                    downloaded_file = await loop.run_in_executor(None, video_dl_sync)
                    direct = True # yt-dlp local download is always direct
                    if not downloaded_file:
                        print(f"yt-dlp fallback failed for {full_link} (video - local download).")
                else:
                    ...
                    if stdout:
                        ...
                        print(f"Successfully retrieved streaming URL via yt-dlp for {full_link}.")
                    else:
                        print(f"yt-dlp failed to get direct URL for {full_link}. Checking file size...")
                        if stderr: # Print stderr if command failed
                            print(f"yt-dlp stderr: {stderr.decode()}")

                        file_size = await check_file_size(full_link)
                        if not file_size:
                            print("File size could not be determined. Returning None.")
                            return None, False
                        total_size_mb = file_size / (1024 * 1024)
                        if total_size_mb > 250: # Enforce 250MB limit
                            print(f"File size {total_size_mb:.2f} MB exceeds the 250MB limit. Not downloading. Returning None.")
                            return None, False
                        ...
                        def video_dl_sync_fallback_size():
                            ...
                            try:
                                ...
                                print(f"yt-dlp video_dl_sync_fallback_size failed for {full_link}: {e}")
                                return None
                        downloaded_file = await loop.run_in_executor(None, video_dl_sync_fallback_size)
                        direct = True # This is a local file
                        if not downloaded_file:
                            print(f"yt-dlp fallback (after size check) failed for {full_link} (video - local download).")


        else: # Case: General audio download (best quality)
            file_ext = "mp3"
            print(f"Trying Maybechiku API for audio: {full_link}")
            downloaded_file, direct = await _download_from_maybechiku_api(full_link, file_ext, videoid)
            ...
            if not downloaded_file: # Fallback to yt-dlp if new API fails or file not found
                ...
                def audio_dl_sync():
                    ...
                    try:
                        ...
                        print(f"yt-dlp audio_dl_sync failed for {full_link}: {e}")
                        return None
                downloaded_file = await loop.run_in_executor(None, audio_dl_sync)
                direct = True # yt-dlp local download is always direct
                if not downloaded_file:
                    print(f"yt-dlp fallback failed for {full_link} (audio).")
